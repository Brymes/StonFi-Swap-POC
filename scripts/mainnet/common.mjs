...
export const NATIVE_TON_ASSET = 'EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c';

export const DEFAULTS = {
  walletName: 'deployer',
  offerJettonAddress: 'EQCxE6mUtQJKFnGfaROTKOt1lZbDiiX1kCixRv7Nw2Id_sDs',
  askAssetAddress: NATIVE_TON_ASSET,
  slippageTolerance: '0.01',
  referralFeeBps: '25',
  fwdGas: '30000000',
  refundFwdGas: '10000000',
  amount: '100000',
  forwardTonAmount: '240000000',
  minOut: null,
  queryId: '42',
...
export function toMainnetAddress(value) {
  const address = value instanceof Address ? value : normalizeAddress(value);
  return address.toString({ testOnly: false });
}

export function isTonAsset(value) {
  const normalized = String(value).trim().toLowerCase();
  return normalized === TON_ASSET || normalized === NATIVE_TON_ASSET.toLowerCase();
}

export function formatAssetAddress(value) {
  if (isTonAsset(value)) {
    return NATIVE_TON_ASSET;
  }
  return toMainnetAddress(value);
}
...
export function extractWalletAddress(wallet) {
  ...
  throw new Error('Unable to find a mainnet wallet address in acton wallet list --json output');
}

export function resolveDeadline(args) {
  if (args['tx-deadline'] !== undefined && args['tx-deadline'] !== '') {
...
export function resolveAddressArg(args, key, fallback) {
  if (args[key]) {
    return toMainnetAddress(args[key]);
  }
  return toMainnetAddress(fallback);
}
